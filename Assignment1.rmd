...
# Power = 0.8
```{r}
power.t.test(delta = 5, sd = 8, sig.level = 0.05, power = 0.8,
                type = "two.sample", alternative = "two.sided", strict = FALSE)
```
# Power = 0.7
```{r}
power.t.test(delta = 5, sd = 8, sig.level = 0.05, power = 0.7,
                type = "two.sample", alternative = "two.sided", strict = FALSE)
```
...
diff_b <- treat_mean_b - none_mean_b
print(diff_b)
```
...
n1 <- sum(data$treatment == 1)
n0 <- sum(data$treatment == 0)
y1 <- data$reduced[data$treatment == 1]
y0 <- data$reduced[data$treatment == 0]
delta_hat <- mean(y1) - mean(y0)
var_hat <- (var(y1) / n1) + (var(y0) / n0)
c(delta_hat - 1.96 * sqrt(var_hat), delta_hat + 1.96 * sqrt(var_hat))
```
...
# Calculating variance estimator:
```{r}
n <- 200
ybar1 <- mean(y1)
ybar0 <- mean(y0)

var_estimator <- ((var(y1) / (0.5 * ybar0^2)) +
                    ((var(y0) * ybar1^2) / (0.5 * ybar0^4))) / n
...
# Calculating 95% CI Interval for estimator
```{r}
delta_hat <- mean(y1) / mean(y0)
c(delta_hat - 1.96 * sqrt(var_estimator),
  delta_hat + 1.96 * sqrt(var_estimator))
```
